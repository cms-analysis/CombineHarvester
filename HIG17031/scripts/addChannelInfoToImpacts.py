...
impacts['checkboxes'] = list(param_dict.keys())
for i, x in enumerate(impacts['params']):
    name = x['name']
    if name in param_dict_rev:
        impacts['params'][i]['checkboxes'] = param_dict_rev[name]
    else:
        impacts['params'][i]['checkboxes'] = list()

    impacts['params'][i]['groups'] = []
...
jsondata = json.dumps(
    impacts, sort_keys=True, indent=2, separators=(',', ': '))
