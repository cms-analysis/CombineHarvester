    ...
    for CHN in ['et', 'mt', 'em', 'tt']:
        for CAT in ['8', '9']:
            ...
            os.system(('python scripts/postFitPlot.py' \
                  ' --file=shapes_160916.root --ratio --extra_pad="%(PAD)s" --mA=1000 --tanb=50 --model_dep' \
...
for MODE in ['prefit']:
    for CHN in ['et', 'mt', 'em', 'tt']:
        for CAT in ['8', '9']:
            LABEL = "%s %s" % (CHN_DICT[CHN], CAT_DICT[CAT])
            os.system(('python scripts/postFitPlot.py' \
                  ' --file=shapes_160916.root --ratio --extra_pad=0.6 --mA=1000 --tanb=50 --model_dep --no_signal' \
