	...
	def doParametersOfInterest(self):
		...
		self.modelBuilder.factory_('expr::ps_scaling("@1*@1-@0*@1*{b_tilde}/{a_tilde}", a, b)'.format(**maxmix))
                ...
                if self.lumi_scale is not None:
                  ...
                  self.modelBuilder.factory_('expr::muF_lumi_scaling("@0*@1", muF, lumi_scaling)'.format(cp=cp))
                  self.modelBuilder.factory_('expr::muV_lumi_scaling("@0*@1", muV, lumi_scaling)'.format(cp=cp))
                  for cp in cps:
