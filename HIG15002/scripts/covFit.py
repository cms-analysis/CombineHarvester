...
def doKappa_gam(wsp,
    ...
    wsp.factory('expr::kappa_gam("sqrt(%s)",%s)' % (kappa_gam, ','.join([k_W, k_t])))
...
def doKappaH(wsp, 
    ...
    kappaHscale = ('0.57*@0*@0 + 0.22*@1*@1 + 0.06*@2*@2 + 0.03*@3*@3 + '
                 ...
                 '0.09*@7*@7 + 0.0023*@8*@8')
    wsp.factory('expr::kappa_H("sqrt(%s)",%s)' % (kappaHscale, ','.join([k_b, k_W, k_tau, k_Z, k_t, k_b, k_tau, k_g, k_gam])))
...
if type == 'B1ZZ_THtoB1WW_TH':
  ...
  for P in ['mu_XS_ggF_x_BR_WW', 'mu_XS_VBF_r_XS_ggF', 'mu_XS_ttH_r_XS_ggF', 'mu_XS_WH_r_XS_ggF', 'mu_XS_ZH_r_XS_ggF', 'mu_BR_tautau_r_BR_WW', 'mu_BR_bb_r_BR_WW', 'mu_BR_gamgam_r_BR_WW', 'mu_BR_ZZ_r_BR_WW']:
    wnew.factory('%s[1,-10.0,10.0]' % P)
  wnew.factory('expr::mu_XS_ggF_x_BR_ZZ("@0*@1", mu_XS_ggF_x_BR_WW, mu_BR_ZZ_r_BR_WW)')
  wnew.factory('expr::mu_BR_WW_r_BR_ZZ("1/@0", mu_BR_ZZ_r_BR_WW)')
  wnew.factory('expr::mu_BR_bb_r_BR_ZZ("@0/@1", mu_BR_bb_r_BR_WW, mu_BR_ZZ_r_BR_WW)')
  wnew.factory('expr::mu_BR_gamgam_r_BR_ZZ("@0/@1", mu_BR_gamgam_r_BR_WW, mu_BR_ZZ_r_BR_WW)')
  wnew.factory('expr::mu_BR_tautau_r_BR_ZZ("@0/@1", mu_BR_tautau_r_BR_WW, mu_BR_ZZ_r_BR_WW)')
  getattr(wnew, 'import')(pdf, ROOT.RooFit.RecycleConflictNodes())
  pdf = wnew.pdf('pdf')
  pdf.Print('tree')
  wfinal = wnew
...
fout = ROOT.TFile('scan_covariance_%s_%s_%s.root' % (args.l, type, POI), 'RECREATE')
