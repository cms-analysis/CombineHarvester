...
namespace ch {
	...
	if (!ttbar_fit){
        ...
        cb.cp().AddSyst(cb, "CMS_htt_eff_b_$ERA", "lnN", SystMap<channel, bin_id, process>::init
                        ({"em"}, {2,3,4,5,6}, {"TTJ","TTT","TT"}, 1.05));

        cb.cp().AddSyst(cb, "CMS_htt_eff_b_$ERA", "lnN", SystMap<channel, bin_id, process>::init
                        ({"em"}, {2, 3,4,5,6}, {"VV","VVT","VVJ"}, 1.015)); // Mainly SingleTop
...
//                                             "CMS_scale_j_$ERA", "shape", SystMap<>::init(1.00));
        ...
        /*for (string uncert:uncertNames){
         ...
         cb.cp().process(JoinStr({sig_procs, {"ZL","ZTT","TTJ","TTT","TT","W","W_rest","ZJ_rest","TTJ_rest","VVJ_rest","VV","VVT","VVJ","ggH_hww125","qqH_hww125","EWKZ"}})).channel({"tt"}).bin_id({12}).AddSyst(cb,
         "CMS_scale_j_"+uncert+"_$ERA", "shape", SystMap<>::init(1.00));
         cb.cp().process(JoinStr({sig_procs, all_mc_bkgs})).channel({"tt"}).bin_id({1,2,3,4,5,6,10,11,13,14,15}).AddSyst(cb,
         "CMS_scale_j_"+uncert+"_$ERA", "shape", SystMap<>::init(1.00));
 
         //Note:  TTJ is excluded from bin 13 of et channel as it was empty and causing error
         cb.cp().process(JoinStr({sig_procs, all_mc_bkgs})).channel({"et"}).bin_id({1,2,3,4,5,6,10,11,12,15}).AddSyst(cb,
         "CMS_scale_j_"+uncert+"_$ERA", "shape", SystMap<>::init(1.00));
         cb.cp().process(JoinStr({sig_procs, all_mc_bkgs_no_W})).channel({"et"}).bin_id({14}).AddSyst(cb,
         "CMS_scale_j_"+uncert+"_$ERA", "shape", SystMap<>::init(1.00));
         if(uncert!="FlavorQCD") {
           cb.cp().process(JoinStr({sig_procs, {"W"}})).channel({"et"}).bin_id({14}).AddSyst(cb,
           "CMS_scale_j_"+uncert+"_$ERA", "shape", SystMap<>::init(1.00));
         }
        ...
        //JES for tt
        // cb.cp().process({"WH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_AbsoluteMPFBias_$ERA", "lnN", SystMap<>::init(1.00590874911));
        // cb.cp().process({"WH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_AbsoluteScale_$ERA", "lnN", SystMap<>::init(1.00590874911));
        // cb.cp().process({"WH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_AbsoluteStat_$ERA", "lnN", SystMap<>::init(1.00590874911));
        // cb.cp().process({"WH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.01244515401));
        // cb.cp().process({"WH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_Fragmentation_$ERA", "lnN", SystMap<>::init(1.00590874911));
        // cb.cp().process({"WH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "lnN", SystMap<>::init(1.00590874911));
        // cb.cp().process({"WH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC1_$ERA", "lnN", SystMap<>::init(1.0065364049));
        // cb.cp().process({"WH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC2_$ERA", "lnN", SystMap<>::init(1.00532999771));
        // cb.cp().process({"WH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtHF_$ERA", "lnN", SystMap<>::init(1.00590874911));
        // cb.cp().process({"WH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtRef_$ERA", "lnN", SystMap<>::init(1.00590874911));
        // cb.cp().process({"WH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.984040861582));
        // cb.cp().process({"WH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeFSR_$ERA", "lnN", SystMap<>::init(1.00590874911));
        // cb.cp().process({"WH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeJERHF_$ERA", "lnN", SystMap<>::init(1.00590874911));
        // cb.cp().process({"WH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC1_$ERA", "lnN", SystMap<>::init(1.0065364049));
        // cb.cp().process({"WH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "lnN", SystMap<>::init(1.01175413407));
        // cb.cp().process({"WH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(1.01148273772));
        // cb.cp().process({"WH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatFSR_$ERA", "lnN", SystMap<>::init(1.00590874911));
        // cb.cp().process({"WH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatHF_$ERA", "lnN", SystMap<>::init(1.00590874911));
        // cb.cp().process({"WH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_SinglePionECAL_$ERA", "lnN", SystMap<>::init(0.994091250891));
        // cb.cp().process({"WH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_SinglePionHCAL_$ERA", "lnN", SystMap<>::init(0.994091250891));
        // cb.cp().process({"WH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_TimePtEta_$ERA", "lnN", SystMap<>::init(1.00590874911));
        // cb.cp().process({"ZH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.00911644818));
        // cb.cp().process({"ZH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.983412519208));
        // cb.cp().process({"ZH_htt125"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(1.00923747942));
        // // cb.cp().process({"ZTT"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.01961480886));
        // cb.cp().process({"ZTT"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "lnN", SystMap<>::init(1.01299795333));
        // cb.cp().process({"ZTT"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC2_$ERA", "lnN", SystMap<>::init(1.00665434016));
        // cb.cp().process({"ZTT"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtHF_$ERA", "lnN", SystMap<>::init(1.00490130831));
        // cb.cp().process({"ZTT"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtRef_$ERA", "lnN", SystMap<>::init(1.00546859182));
        // cb.cp().process({"ZTT"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.968785018213));
        // cb.cp().process({"ZTT"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeFSR_$ERA", "lnN", SystMap<>::init(0.995177865431));
        // cb.cp().process({"ZTT"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "lnN", SystMap<>::init(1.00901271758));
        // cb.cp().process({"ZTT"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(1.01778152211));
        // cb.cp().process({"ZTT"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatHF_$ERA", "lnN", SystMap<>::init(1.00507694785));
        // cb.cp().process({"ZTT"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_TimePtEta_$ERA", "lnN", SystMap<>::init(0.995758100964));
        // cb.cp().process({"TTT"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.01324266977));
        // cb.cp().process({"TTT"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC1_$ERA", "lnN", SystMap<>::init(1.01324266977));
        // cb.cp().process({"TTT"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(1.01324266977));
        // cb.cp().process({"TTT"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC1_$ERA", "lnN", SystMap<>::init(1.01324266977));
        ...
        // cb.cp().process({"VVJ"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(1.01304150742));
        // cb.cp().process({"VVJ"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "lnN", SystMap<>::init(1.00550070401));
        // cb.cp().process({"VVJ"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(1.00258860054));
        // cb.cp().process({"VVT"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.0054169136));
        // cb.cp().process({"VVT"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.970464556154));
        // cb.cp().process({"VVT"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(1.0127700062));
        // cb.cp().process({"ZL"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC2_$ERA", "lnN", SystMap<>::init(1.04978898077));
        // cb.cp().process({"ZL"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "lnN", SystMap<>::init(1.04978898077));
        ...
        // 
        // //JES for mt
        // cb.cp().process({"WH_htt125"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.00875543073));
        // cb.cp().process({"WH_htt125"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC2_$ERA", "lnN", SystMap<>::init(1.00340771995));
        // cb.cp().process({"WH_htt125"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtHF_$ERA", "lnN", SystMap<>::init(1.00308758623));
        // cb.cp().process({"WH_htt125"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.989422208727));
        // cb.cp().process({"WH_htt125"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"WH_htt125"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(1.00718285709));
        // cb.cp().process({"ZH_htt125"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"ZH_htt125"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC2_$ERA", "lnN", SystMap<>::init(1.00358859032));
        // cb.cp().process({"ZH_htt125"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"ZH_htt125"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "lnN", SystMap<>::init(1.00380986955));
        // cb.cp().process({"ZH_htt125"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(1.00285619361));
        // cb.cp().process({"ZTT"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.01039940134));
        // cb.cp().process({"ZTT"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "lnN", SystMap<>::init(1.00643274475));
        // cb.cp().process({"ZTT"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.971191744164));
        // cb.cp().process({"ZTT"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"ZTT"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(1.01347393224));
        // cb.cp().process({"TTT"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.993477456034));
        // cb.cp().process({"TTJ"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"TTJ"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "shape", SystMap<>::init(1.00));
        ...
        // cb.cp().process({"W"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC2_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"W"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtHF_$ERA", "lnN", SystMap<>::init(1.00706895106));
        // cb.cp().process({"W"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtRef_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"W"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.966078565099));
        // cb.cp().process({"W"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"W"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(1.0089091179));
        // cb.cp().process({"W"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_SinglePionECAL_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"VV"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.00633449848));
        // cb.cp().process({"VV"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "lnN", SystMap<>::init(1.00336332746));
        // cb.cp().process({"VV"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC2_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"VV"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.976932854818));
        // cb.cp().process({"VV"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"VV"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(1.00837233812));
        // cb.cp().process({"ZL"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.02023064736));
        // cb.cp().process({"ZL"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "lnN", SystMap<>::init(1.02023064736));
        // cb.cp().process({"ZL"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC2_$ERA", "lnN", SystMap<>::init(1.00728993949));
        // cb.cp().process({"ZL"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtHF_$ERA", "lnN", SystMap<>::init(1.02023064736));
        // cb.cp().process({"ZL"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtRef_$ERA", "lnN", SystMap<>::init(1.02023064736));
        // cb.cp().process({"ZL"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.947513030852));
        // cb.cp().process({"ZL"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeFSR_$ERA", "lnN", SystMap<>::init(0.979769352641));
        // cb.cp().process({"ZL"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "lnN", SystMap<>::init(1.01591552631));
        // cb.cp().process({"ZL"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(1.03289938497));
        // cb.cp().process({"ZL"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatHF_$ERA", "lnN", SystMap<>::init(1.02023064736));
        // cb.cp().process({"ZJ"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.01402620293));
        // cb.cp().process({"ZJ"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "lnN", SystMap<>::init(1.01402620293));
        // cb.cp().process({"ZJ"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC2_$ERA", "lnN", SystMap<>::init(1.01402620293));
        // cb.cp().process({"ZJ"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.914534696061));
        // cb.cp().process({"ZJ"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "lnN", SystMap<>::init(1.01402620293));
        // cb.cp().process({"ZJ"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(1.01470862451));
        // cb.cp().process({"ZJ"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatEC_$ERA", "lnN", SystMap<>::init(1.01402620293));
        // cb.cp().process({"ZJ"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_TimePtEta_$ERA", "lnN", SystMap<>::init(0.985973797065));
        cb.cp().process({"QCD"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.00503474319));
        cb.cp().process({"QCD"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(1.01313336657));
        cb.cp().process({"QCD"}).channel({"mt"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(0.996268355665));
        ...
        // 
        // //JES for et
        // cb.cp().process({"WH_htt125"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.0048383722));
        // cb.cp().process({"WH_htt125"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "lnN", SystMap<>::init(1.0048383722));
        // cb.cp().process({"WH_htt125"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtHF_$ERA", "lnN", SystMap<>::init(1.0037164185));
        // cb.cp().process({"WH_htt125"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtRef_$ERA", "lnN", SystMap<>::init(1.0048383722));
        // cb.cp().process({"WH_htt125"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.978975501165));
        // cb.cp().process({"WH_htt125"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "lnN", SystMap<>::init(1.00381338592));
        // cb.cp().process({"WH_htt125"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(1.00659462719));
        // cb.cp().process({"WH_htt125"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatHF_$ERA", "lnN", SystMap<>::init(1.0037164185));
        // cb.cp().process({"WH_htt125"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_TimePtEta_$ERA", "lnN", SystMap<>::init(1.0025944648));
        // cb.cp().process({"ZH_htt125"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.01378219704));
        // cb.cp().process({"ZH_htt125"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "lnN", SystMap<>::init(1.00355649047));
        // cb.cp().process({"ZH_htt125"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC1_$ERA", "lnN", SystMap<>::init(1.00472408872));
        // cb.cp().process({"ZH_htt125"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC2_$ERA", "lnN", SystMap<>::init(1.00550161786));
        // cb.cp().process({"ZH_htt125"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtHF_$ERA", "lnN", SystMap<>::init(1.00355649047));
        // cb.cp().process({"ZH_htt125"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.974333798522));
        // cb.cp().process({"ZH_htt125"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeFSR_$ERA", "lnN", SystMap<>::init(0.990941891677));
        // cb.cp().process({"ZH_htt125"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC1_$ERA", "lnN", SystMap<>::init(1.00472408872));
        // cb.cp().process({"ZH_htt125"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "lnN", SystMap<>::init(1.00550161786));
        // cb.cp().process({"ZH_htt125"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(1.00817876323));
        // cb.cp().process({"ZTT"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.01606132087));
        // cb.cp().process({"ZTT"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "lnN", SystMap<>::init(1.00837305724));
        // cb.cp().process({"ZTT"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC1_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"ZTT"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC2_$ERA", "lnN", SystMap<>::init(1.00447312882));
        // cb.cp().process({"ZTT"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtRef_$ERA", "lnN", SystMap<>::init(1.00524154876));
        // cb.cp().process({"ZTT"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.972907339731));
        // cb.cp().process({"ZTT"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC1_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"ZTT"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "lnN", SystMap<>::init(1.00569765039));
        ...
        // cb.cp().process({"W"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "lnN", SystMap<>::init(1.01523211383));
        // cb.cp().process({"W"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC1_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"W"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC2_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"W"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtHF_$ERA", "lnN", SystMap<>::init(1.00999380281));
        // cb.cp().process({"W"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtRef_$ERA", "lnN", SystMap<>::init(1.01315940828));
        // cb.cp().process({"W"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.93919891208));
        // cb.cp().process({"W"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"W"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(1.03770578358));
        // cb.cp().process({"W"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatEC_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"W"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatFSR_$ERA", "lnN", SystMap<>::init(1.00493370182));
        // cb.cp().process({"W"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatHF_$ERA", "lnN", SystMap<>::init(1.01530808176));
        // cb.cp().process({"W"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_SinglePionECAL_$ERA", "lnN", SystMap<>::init(0.995614418117));
        // cb.cp().process({"W"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_SinglePionHCAL_$ERA", "lnN", SystMap<>::init(0.995420947555));
        // cb.cp().process({"VV"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_AbsoluteMPFBias_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"VV"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_AbsoluteScale_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"VV"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.01062418327));
        // cb.cp().process({"VV"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_Fragmentation_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"VV"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"VV"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtHF_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"VV"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtRef_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"VV"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.977113288947));
        // cb.cp().process({"VV"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeFSR_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"VV"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(1.01451084244));
        // cb.cp().process({"VV"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatFSR_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"VV"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatHF_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"VV"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_SinglePionECAL_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"VV"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_SinglePionHCAL_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"ZL"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_AbsoluteMPFBias_$ERA", "lnN", SystMap<>::init(1.0054388913));
        // cb.cp().process({"ZL"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_AbsoluteScale_$ERA", "lnN", SystMap<>::init(1.0054388913));
        // cb.cp().process({"ZL"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_AbsoluteStat_$ERA", "lnN", SystMap<>::init(1.0054388913));
        // cb.cp().process({"ZL"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.02826481265));
        // cb.cp().process({"ZL"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_Fragmentation_$ERA", "lnN", SystMap<>::init(1.0054388913));
        // cb.cp().process({"ZL"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "lnN", SystMap<>::init(1.02099955212));
        // cb.cp().process({"ZL"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC1_$ERA", "lnN", SystMap<>::init(1.00761191381));
        // cb.cp().process({"ZL"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC2_$ERA", "lnN", SystMap<>::init(1.00726526053));
        // cb.cp().process({"ZL"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtHF_$ERA", "lnN", SystMap<>::init(1.01338763831));
        // cb.cp().process({"ZL"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtRef_$ERA", "lnN", SystMap<>::init(1.01305080511));
        // cb.cp().process({"ZL"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.946369075612));
        // cb.cp().process({"ZL"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeFSR_$ERA", "lnN", SystMap<>::init(0.986612361691));
        // cb.cp().process({"ZL"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC1_$ERA", "lnN", SystMap<>::init(1.00761191381));
        // cb.cp().process({"ZL"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "lnN", SystMap<>::init(1.01738687126));
        // cb.cp().process({"ZL"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(1.0304988994));
        // cb.cp().process({"ZL"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatFSR_$ERA", "lnN", SystMap<>::init(1.0054388913));
        // cb.cp().process({"ZL"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatHF_$ERA", "lnN", SystMap<>::init(1.0054388913));
        // cb.cp().process({"ZL"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_SinglePionECAL_$ERA", "lnN", SystMap<>::init(0.986949194891));
        // cb.cp().process({"ZL"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_SinglePionHCAL_$ERA", "lnN", SystMap<>::init(0.994561108704));
        // cb.cp().process({"ZJ"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.03461762363));
        // cb.cp().process({"ZJ"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "lnN", SystMap<>::init(1.0159274809));
        // cb.cp().process({"ZJ"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC1_$ERA", "lnN", SystMap<>::init(1.01063778335));
        // cb.cp().process({"ZJ"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC2_$ERA", "lnN", SystMap<>::init(1.00703035708));
        // cb.cp().process({"ZJ"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtHF_$ERA", "lnN", SystMap<>::init(1.00889712383));
        // cb.cp().process({"ZJ"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtRef_$ERA", "lnN", SystMap<>::init(1.00703035708));
        // cb.cp().process({"ZJ"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.961972008201));
        // cb.cp().process({"ZJ"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeFSR_$ERA", "lnN", SystMap<>::init(0.992969642923));
        // cb.cp().process({"ZJ"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC1_$ERA", "lnN", SystMap<>::init(1.01063778335));
        // cb.cp().process({"ZJ"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "lnN", SystMap<>::init(1.01397819542));
        // cb.cp().process({"ZJ"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(1.02361397854));
        // cb.cp().process({"ZJ"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatEC_$ERA", "lnN", SystMap<>::init(1.00703035708));
        cb.cp().process({"QCD"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "shape", SystMap<>::init(1.00));
        cb.cp().process({"QCD"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(1.01939374359));
        cb.cp().process({"QCD"}).channel({"et"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "shape", SystMap<>::init(1.00));
        ...
        // 
        // //JES for em
        // cb.cp().process({"WH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_AbsoluteStat_$ERA", "lnN", SystMap<>::init(1.00676941355));
        // cb.cp().process({"WH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.0014575809));
        // cb.cp().process({"WH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "lnN", SystMap<>::init(0.99518590626));
        // cb.cp().process({"WH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtBB_$ERA", "lnN", SystMap<>::init(0.993847147922));
        // cb.cp().process({"WH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC2_$ERA", "lnN", SystMap<>::init(1.02328861079));
        // cb.cp().process({"WH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtHF_$ERA", "lnN", SystMap<>::init(0.991796579248));
        // cb.cp().process({"WH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtRef_$ERA", "lnN", SystMap<>::init(0.996240670101));
        // cb.cp().process({"WH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.969793313569));
        // cb.cp().process({"WH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeFSR_$ERA", "lnN", SystMap<>::init(1.0096011381));
        // cb.cp().process({"WH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtBB_$ERA", "lnN", SystMap<>::init(0.993847147922));
        // cb.cp().process({"WH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "lnN", SystMap<>::init(1.01580583282));
        // cb.cp().process({"WH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(0.995370373676));
        // cb.cp().process({"WH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatEC_$ERA", "lnN", SystMap<>::init(1.00954217909));
        // cb.cp().process({"WH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatFSR_$ERA", "lnN", SystMap<>::init(1.00676941355));
        // cb.cp().process({"WH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatHF_$ERA", "lnN", SystMap<>::init(0.995772592208));
        // cb.cp().process({"WH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_SinglePionECAL_$ERA", "lnN", SystMap<>::init(1.0037593299));
        // cb.cp().process({"WH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_TimePtEta_$ERA", "lnN", SystMap<>::init(0.99760647782));
        // cb.cp().process({"ZH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.00960045445));
        // cb.cp().process({"ZH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "lnN", SystMap<>::init(0.991432204849));
        // cb.cp().process({"ZH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtHF_$ERA", "lnN", SystMap<>::init(0.991432204849));
        // cb.cp().process({"ZH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(1.00848545696));
        // cb.cp().process({"ZH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC1_$ERA", "lnN", SystMap<>::init(1.0181682496));
        // cb.cp().process({"ZH_htt125"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(0.991432204849));
        // cb.cp().process({"ZTT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_AbsoluteMPFBias_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"ZTT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_AbsoluteScale_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"ZTT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_AbsoluteStat_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"ZTT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.0192949246));
        // cb.cp().process({"ZTT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_Fragmentation_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"ZTT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "lnN", SystMap<>::init(1.01658363571));
        // cb.cp().process({"ZTT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC2_$ERA", "lnN", SystMap<>::init(1.01178102352));
        // cb.cp().process({"ZTT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtHF_$ERA", "lnN", SystMap<>::init(1.009558801));
        // cb.cp().process({"ZTT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtRef_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"ZTT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.909240354708));
        // cb.cp().process({"ZTT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeFSR_$ERA", "lnN", SystMap<>::init(0.995941356151));
        // cb.cp().process({"ZTT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "lnN", SystMap<>::init(1.01471908584));
        // cb.cp().process({"ZTT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(1.04070986812));
        // cb.cp().process({"ZTT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatEC_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"ZTT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatFSR_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"ZTT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatHF_$ERA", "lnN", SystMap<>::init(1.00925393377));
        // cb.cp().process({"ZTT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_SinglePionECAL_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"ZTT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_SinglePionHCAL_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"ZTT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_TimePtEta_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"TT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_AbsoluteMPFBias_$ERA", "lnN", SystMap<>::init(0.993680409313));
        // cb.cp().process({"TT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(0.979004019653));
        // cb.cp().process({"TT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_Fragmentation_$ERA", "lnN", SystMap<>::init(0.993689739712));
        // cb.cp().process({"TT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "lnN", SystMap<>::init(0.994453995785));
        // cb.cp().process({"TT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtRef_$ERA", "lnN", SystMap<>::init(0.99061611512));
        // cb.cp().process({"TT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.981443072596));
        // cb.cp().process({"TT"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "shape", SystMap<>::init(1.00));
        ...
        // cb.cp().process({"W"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.03310161245));
        // cb.cp().process({"W"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_Fragmentation_$ERA", "lnN", SystMap<>::init(1.00508816404));
        // cb.cp().process({"W"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "lnN", SystMap<>::init(1.0328056197));
        // cb.cp().process({"W"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC2_$ERA", "lnN", SystMap<>::init(1.02919732423));
        // cb.cp().process({"W"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtHF_$ERA", "lnN", SystMap<>::init(1.00411471437));
        // cb.cp().process({"W"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtRef_$ERA", "lnN", SystMap<>::init(1.0328056197));
        // cb.cp().process({"W"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.914088576968));
        // cb.cp().process({"W"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeFSR_$ERA", "lnN", SystMap<>::init(1.03081343036));
        // cb.cp().process({"W"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeJERHF_$ERA", "lnN", SystMap<>::init(1.00311861971));
        // cb.cp().process({"W"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "lnN", SystMap<>::init(1.02919732423));
        // cb.cp().process({"W"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(0.968099041941));
        // cb.cp().process({"W"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatEC_$ERA", "lnN", SystMap<>::init(1.02869090533));
        // cb.cp().process({"W"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatFSR_$ERA", "lnN", SystMap<>::init(1.00411471437));
        // cb.cp().process({"W"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatHF_$ERA", "lnN", SystMap<>::init(1.00411471437));
        // cb.cp().process({"W"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_SinglePionECAL_$ERA", "lnN", SystMap<>::init(0.994911835964));
        // cb.cp().process({"W"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_SinglePionHCAL_$ERA", "lnN", SystMap<>::init(0.994911835964));
        // cb.cp().process({"W"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_TimePtEta_$ERA", "lnN", SystMap<>::init(0.973431619713));
        // cb.cp().process({"VV"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.01102597069));
        // cb.cp().process({"VV"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "lnN", SystMap<>::init(1.00933936271));
        // cb.cp().process({"VV"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC1_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"VV"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC2_$ERA", "lnN", SystMap<>::init(1.00414979416));
        // cb.cp().process({"VV"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtHF_$ERA", "lnN", SystMap<>::init(1.00634593132));
        // cb.cp().process({"VV"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtRef_$ERA", "lnN", SystMap<>::init(1.00612778437));
        // cb.cp().process({"VV"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.961878881892));
        // cb.cp().process({"VV"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC1_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"VV"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "lnN", SystMap<>::init(1.00419541212));
        // cb.cp().process({"VV"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "lnN", SystMap<>::init(1.02205871779));
        // cb.cp().process({"VV"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatEC_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"VV"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatFSR_$ERA", "lnN", SystMap<>::init(1.00482506075));
        // cb.cp().process({"VV"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatHF_$ERA", "lnN", SystMap<>::init(1.0062174462));
        // cb.cp().process({"ZL"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "lnN", SystMap<>::init(1.1250269225));
        // cb.cp().process({"ZL"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "lnN", SystMap<>::init(1.06465927863));
        // cb.cp().process({"ZL"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC2_$ERA", "lnN", SystMap<>::init(1.06465927863));
        // cb.cp().process({"ZL"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "lnN", SystMap<>::init(0.934567185869));
        // cb.cp().process({"ZL"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "lnN", SystMap<>::init(1.06465927863));
        // cb.cp().process({"ZL"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeStatEC_$ERA", "lnN", SystMap<>::init(1.06465927863));
        // cb.cp().process({"ZL"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_TimePtEta_$ERA", "lnN", SystMap<>::init(0.935340721368));
        // cb.cp().process({"QCD"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_AbsoluteMPFBias_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"QCD"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_FlavorQCD_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"QCD"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_Fragmentation_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"QCD"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpDataMC_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"QCD"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtBB_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"QCD"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtEC1_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"QCD"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_PileUpPtRef_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"QCD"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeBal_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"QCD"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativeFSR_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"QCD"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtEC2_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"QCD"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_RelativePtHF_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"QCD"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_SinglePionECAL_$ERA", "shape", SystMap<>::init(1.00));
        // cb.cp().process({"QCD"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,"CMS_scale_j_SinglePionHCAL_$ERA", "shape", SystMap<>::init(1.00));
...
//                                                        "CMS_htt_eFakeTau_13TeV", "lnN", SystMap<>::init(1.12));
        ...
        cb.cp().process( {"ZL"}).channel({"mt"}).bin_id({1,2,3,4,5,6}).AddSyst(cb,
                                                                     "CMS_mFakeTau_1prong_$ERA", "shape", SystMap<>::init(1.00));
        cb.cp().process( {"ZL"}).channel({"mt"}).bin_id({1,2,3,4,5,6}).AddSyst(cb,
                                                                     "CMS_mFakeTau_1prong1pizero_$ERA", "shape", SystMap<>::init(1.00));
        cb.cp().process( {"ZL"}).channel({"et"}).bin_id({1,2,3,4,5,6}).AddSyst(cb,
...
//        
        ...
        */
        ...
        else
        {
            ...
            cb.cp().process( {"ZL","ZTT","ZJ", "ZJ_rest", "EWKZ"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,
                                             "CMS_htt_zmm_norm_extrap_VBF_em_$ERA", "lnN", SystMap<>::init(1.15));
            cb.cp().process( {"ZL","ZTT","ZJ", "ZJ_rest", "EWKZ"}).channel({"et","mt"}).bin_id({3,4,5,6}).AddSyst(cb,
                                             "CMS_htt_zmm_norm_extrap_VBF_lt_$ERA", "lnN", SystMap<>::init(1.15));
            cb.cp().process( {"ZL","ZTT","ZJ", "ZJ_rest", "EWKZ"}).channel({"tt"}).bin_id({3,4,5,6}).AddSyst(cb,
...
//                                            "CMS_htt_zmumuShape_VBF_$ERA", "shape", SystMap<>::init(1.00));
            ...
            cb.cp().process( {"ZL","ZTT","ZJ", "ZJ_rest", "EWKZ"}).channel({"tt","et","mt"}).bin_id({3,4,5,6}).AddSyst(cb,
                                            "CMS_htt_zmumuShape_VBF_$ERA", "shape", SystMap<>::init(1.00));
            cb.cp().process( {"ZL","ZTT","ZJ", "ZJ_rest"}).channel({"em"}).bin_id({3,4,5,6}).AddSyst(cb,
                                            "CMS_htt_zmumuShape_VBF_$ERA", "shape", SystMap<>::init(1.00));
