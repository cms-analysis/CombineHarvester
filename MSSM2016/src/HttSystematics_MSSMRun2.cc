...
/*  cb.cp().process({"TTH_SM125"}).AddSyst(cb, "pdf_Higgs_ttH","lnN",
  ...
  cb.cp().AddSyst(cb,
    ...
    ({"mt"}, {8}, {"TTT","TTJ"},   1.014,  0.986)
    ({"mt"}, {8}, {bbH},  1.01 ,  0.990)
    ({"mt"}, {9}, {"ZL"},   0.97 ,  0.91)
    ({"mt"}, {9}, {"ZJ"},   0.968,  0.960)
    ({"mt"}, {9}, {"ZTT"},  0.969,  1.026)
    ({"mt"}, {9}, {"TTT","TTJ"},   1.092,  0.917)
    ({"mt"}, {9}, {"VVT","VVJ"},   1.0415, 0.979)
    ({"mt"}, {9}, {ggH},  1,      0.94)
    ({"mt"}, {9}, {bbH},  1.02,   0.98)
    ({"et"}, {8}, {"TTT","TTJ"},   1.013,  0.987)
    ({"et"}, {8}, {bbH},  1.01,   0.99)
    ({"et"}, {9}, {"ZL"},   0.983,  1.028)
    ({"et"}, {9}, {"ZJ"},   1.001,  1.112)
    ({"et"}, {9}, {"ZTT"},  0.989,  1.015)
    ({"et"}, {9}, {"VVT","VVJ"},   1.037,  0.955)
    ({"et"}, {9}, {"TTT","TTJ"},   1.0978, 0.9112)
    ({"et"}, {9}, {ggH},  1.06,   0.96)
    ({"et"}, {9}, {ggH},  0.98,   1)
    ({"em"}, {8}, {"TT"},   1.015,  0.986)
    ({"em"}, {8}, {bbH},  1.01,   0.99)
    ...
    ({"tt"}, {8}, {"ZJ"},   1.01,   0.99)
    ({"tt"}, {8}, {"ZTT"},  1.01,   0.99)
    ({"tt"}, {8}, {"VVT","VVJ"},   1.01,   0.97)
    ({"tt"}, {8}, {"TTT","TTJ"},   1.04,   0.96)
    ({"tt"}, {8}, {ggH},  1.01,   0.99)
    ({"tt"}, {8}, {bbH},  1.02,   0.98)
    ({"tt"}, {9}, {"W"},    1.06,   1.23)
    ...
    ({"tt"}, {9}, {"ZJ"},   1.16,   1.11)
    ({"tt"}, {9}, {"ZTT"},  0.96,   1.04)
    ({"tt"}, {9}, {"VVT","VVJ"},   1.05,   1.08)
    ({"tt"}, {9}, {"TTT","TTJ"},   1.08,   0.92)
    ({"tt"}, {9}, {bbH},  0.98,   1.04)
    ({"tt"}, {9}, {ggH},  0.98,   1.02));
  ...
  cb.cp().AddSyst(cb,
    ...
    // ({"mt"},{8}, {"W"}, 0.99,1.01)
    ({"mt"}, {8}, {"VVT","VVJ"},   1.01,   0.99)
    ({"mt"}, {8}, {"TTT","TTJ"},   1.06,   0.95)
    ({"mt"}, {8}, {bbH},  1.01,   0.99)
    // ({"mt"},{9}, {"W"}, 1.41,0.62)
    // ({"mt"},{9}, {"QCD"}, 0.99,1.01)
    ({"mt"}, {9}, {"ZL"},   0.98,   1.02)
    ...
    ({"mt"}, {9}, {"ZTT"},  0.97,   1.03)
    ({"mt"}, {9}, {"VV"},   0.98,   1.03)
    ({"mt"}, {9}, {"TTT","TTJ"},   0.98,   1.02)
    ({"mt"}, {9}, {ggH},  0.98,   1)
    ({"mt"}, {9}, {bbH},  0.97,   1.04)
    // ({"et"},{8}, {"W"}, 0.99,1.01)
    ({"et"}, {8}, {"VVT","VVJ"},   1.01,   0.99)
    ({"et"}, {8}, {"TTT","TTJ"},   1.05,   0.95)
    ({"et"}, {8}, {bbH},  1.01,   0.99)
    // ({"et"},{9}, {"W"}, 1.19,0.91)
    // ({"et"},{9}, {"QCD"}, 0.99,1)
    ({"et"}, {9}, {"ZL"},   0.99,   1.03)
    ({"et"}, {9}, {"ZJ"},   0.96,   1.3)
    ({"et"}, {9}, {"ZTT"},  0.96,   1.03)
    ({"et"}, {9}, {"VVT","VVJ"},   0.97,   1.03)
    ({"et"}, {9}, {"TTT","TTJ"},   0.98,   1.02)
    ({"et"}, {9}, {ggH},  0.96,   1.03)
    ({"et"}, {9}, {bbH},  0.98,   1.03)
    ({"em"}, {8}, {"VV"},   1.01,   0.99)
    ({"em"}, {8}, {"TT"},   1.06,   0.95)
    ...
    ({"tt"}, {8}, {"W"},    1,      0.99)
    ({"tt"}, {8}, {"ZL"},   1.01,   0.99)
    ({"tt"}, {8}, {"TTT","TTJ"},   1.03,   0.92)
    ({"tt"}, {8}, {bbH},  1.01,   0.99)
    ({"tt"}, {9}, {"W"},    1,      1.1)
    ({"tt"}, {9}, {"ZL"},   0.93,   1.03)
    ({"tt"}, {9}, {"ZTT"},  0.99,   1.01)
    ({"tt"}, {9}, {"TTT","TTJ"},   0.99,   1.01)
    ({"tt"}, {9}, {bbH},  0.98,   1.02));
  ...
  cb.cp().AddSyst(cb,
    ...
    ({"em"}, {9}, {"ZTT"},  0.99,   1.01)
    ({"em"}, {9}, {ggH},  0.97,   1.04)
    ({"tt"}, {8}, {"TTT","TTJ"},   1.01,   0.998)
    ({"tt"}, {8}, {ggH},  1.01,   0.99)
    ({"tt"}, {9}, {"ZL"},   0.96,   1.03)
    ({"tt"}, {9}, {"ZTT"},  0.98,   1.03)
    ({"tt"}, {9}, {"VVT","VVJ"},   1.01,   1.03)
    ({"tt"}, {9}, {ggH},  0.97,   1.03));
    ...
    if (control_region == 1) {
      ...
      for (auto bin : cb_sig.cp().channel({"et", "mt"}).bin_set()) {
        ...
        cb.cp().AddSyst(cb,
          ...
          ({"mt"},  {10}, {"TTT","TTJ"}, 1.017, 0.986)
          ({"mt"},  {11}, {"TTT","TTJ"}, 1.009, 0.992)
          ({"mt"},  {12}, {"TTT","TTJ"}, 1.011, 0.989)
          ({"mt"},  {13}, {"TTT","TTJ"}, 1.076, 0.924)
          ({"mt"},  {13}, {"VVT","VVJ"}, 1.014, 0.978)
          ({"mt"},  {14}, {"ZJ"}, 0.971, 1.021)
          ({"mt"},  {14}, {"TTT","TTJ"}, 1.132, 0.879)
          ({"mt"},  {15}, {"TTT","TTJ"}, 1.089, 0.918)
          ({"et"},  {10}, {"TTT","TTJ"}, 1.017, 0.985)
          ({"et"},  {11}, {"TTT","TTJ"}, 1.014, 0.988)
          ({"et"},  {12}, {"TTT","TTJ"}, 1.011, 0.988)
          ({"et"},  {13}, {"TTT","TTJ"}, 1.073, 0.927)
          ({"et"},  {13}, {"VVT","VVJ"}, 1.032, 0.972)
          ({"et"},  {14}, {"TTT","TTJ"}, 1.099, 0.896)
          ({"et"},  {15}, {"TTT","TTJ"}, 1.086, 0.924));

        cb.cp().AddSyst(cb,
          "CMS_eff_b_13TeV", "lnN", SystMapAsymm<channel,bin_id,process>::init
          ({"mt"},  {10}, {"TTT","TTJ"}, 1.054, 0.951)
          ({"mt"},  {11}, {"TTT","TTJ"}, 1.035, 0.966)
          ({"mt"},  {12}, {"TTT","TTJ"}, 1.036, 0.963)
          ({"mt"},  {13}, {"TTT","TTJ"}, 0.978, 1.020)
          ({"mt"},  {13}, {"VVT","VVJ"}, 0.972, 1.029)
          ({"mt"},  {14}, {"TTT","TTJ"}, 0.975, 1.026)
          ({"mt"},  {15}, {"TTT","TTJ"}, 0.972, 1.022)
          ({"mt"},  {15}, {"VVT","VVJ"}, 0.949, 1.036)
          ({"et"},  {10}, {"TTT","TTJ"}, 1.054, 0.949)
          ({"et"},  {11}, {"TTT","TTJ"}, 1.043, 0.961)
          ({"et"},  {12}, {"TTT","TTJ"}, 1.039, 0.964)
          ({"et"},  {13}, {"TTT","TTJ"}, 0.978, 1.022)
          ({"et"},  {14}, {"TTT","TTJ"}, 0.957, 1.022)
          ({"et"},  {15}, {"TTT","TTJ"}, 0.974, 1.023));
      }
