...
#	echo $lterm
	python scripts/plotMSSMLimits.py --logy --logx --show exp0 $lterm --cms-sub="Preliminary" -o ${o}/${m}_${p} --process=${p:0:2}'#phi' --title-right="13 TeV" --use-hig-17-020-style --auto-style --ratio-to ${ds_[0]}${p}'_cmb.json:exp0' --title-left="$title_left"
    done
done
...
#	echo $lterm
	python scripts/plotMSSMLimits.py --logy --logx --show exp0 $lterm --cms-sub="Preliminary" -o ${o}/${lumititle}fb_${p} --process=${p:0:2}'#phi' --title-right="13 TeV" --use-hig-17-020-style --auto-style --ratio-to ${ds_[0]}${p}'_cmb.json:exp0' --title-left="$title_left"
    done
done
