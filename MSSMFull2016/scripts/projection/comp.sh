...
#	title_left=${p}, `echo $m | tr - ' '`
	title_left="${p}, `echo $m | tr - ' '`"
	echo "##### $title_left #####"
	python scripts/plotMSSMLimits.py --logy --logx --show exp0 ${ds_[0]}${p}'_cmb.json:exp0:Title="36.9 fb^{-1}"' ${ds_[1]}${p}'_cmb.json:exp0:Title="300 fb^{-1}"' ${ds_[2]}${p}'_cmb.json:exp0:Title="3000 fb^{-1}"' --cms-sub="Preliminary" -o output/${m}_${p} --process=${p:0:2}'#phi' --title-right="13 TeV" --use-hig-17-020-style --auto-style --ratio-to ${ds_[0]}${p}'_cmb.json:exp0' --title-left="$title_left"
#	python scripts/plotMSSMLimits.py --logy --logx --show exp0 ${ds_[0]}${p}'_cmb.json:exp0:Title="36.9 fb^{-1}"' ${ds_[1]}${p}'_cmb.json:exp0:Title="300/fb"' ${ds_[2]}${p}'_cmb.json:exp0:Title="3000/fb"' --cms-sub="Preliminary" -o output/${m}_${p} --process=${p:0:2}'#phi' --title-right="13 TeV" --use-hig-17-020-style --auto-style --ratio-to ${ds_[0]}${p}'_cmb.json:exp0' --title-left='${p}', '`echo $m | tr - ' '`\'
    done
done
