...
for i in range(fpf_s.getSize()):
    ...
    if nuis_p == None:
        ...
        try:
            ...
            print "No value or error for",name
            hasError = True
            if not options.abs: continue
            row += [ " n/a " ]
...
if options.plotfile:
    ...
    for b in range(1,hist_fit_e_s.GetNbinsX()+1): 
      scaleby=hist_prefit.GetBinError(b)
      if not scaleby>0: scaleby=1
      hist_fit_e_s.SetBinContent(b,hist_fit_s.GetBinError(b)/scaleby)
      hist_fit_e_b.SetBinContent(b,hist_fit_b.GetBinError(b)/scaleby)
