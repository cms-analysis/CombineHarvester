...
/*  cb.cp().process(ch::JoinStr({signal, {"TTT","TTJ","VVJ","ZL","ZJ","ZTT","W","VV","TT","ZLL","QCD"}}))
...
//  cb.cp().process(JoinStr({signal, {"ZTT","TTT"}})).channel({"et","mt","tt"}).AddSyst(cb,
...
//    ({"et", "mt", "em", "tt"}, {1, 2, 3, 4,5,6}, {"TT","VV"}, 1.02));
//
//  cb.cp().AddSyst(cb,
...
//    ({"et", "mt", "em", "tt"}, {1, 2, 3, 4,5,6}, {"TT","VV"}, 1.02));
  ...
  // ------------------------
//  cb.cp().channel({"zmm"},false).process({"TT"}).AddSyst(cb,
