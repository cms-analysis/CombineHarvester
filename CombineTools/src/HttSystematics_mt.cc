...
void AddSystematics_ee_mm(CombineHarvester & cb) {
  ...
  src.cp().process({"Dibosons"}).AddSyst(cb,
      "CMS_htt_$CHANNEL_DiBosonNorm_$ERA", "lnN", SystMap<>::init(1.30));
...
void AddSystematics_em(CombineHarvester & cb) {
  ...
  std::vector<std::string> ggH = {"ggH", "ggH_hww125"};
  std::vector<std::string> qqH = {"ggH", "ggH_hww125"};

  auto signal = Set2Vec(src.cp().signals().process_set());

  src.cp().process_rgx({higgs_rgx, "EWK"})
      .AddSyst(cb, "lumi_$ERA", "lnN", SystMap<era>::init
        ({"7TeV"}, 1.022)
        ({"8TeV"}, 1.026));

  src.cp().AddSyst(cb, "CMS_eff_e", "lnN", SystMap<>::init(1.02));
  src.cp().AddSyst(cb, "CMS_eff_m", "lnN", SystMap<>::init(1.02));

  src.cp().process_rgx({higgs_rgx, "Ztt"}).bin_id({0, 2, 4, 5})
      .AddSyst(cb, "CMS_scale_e_$ERA", "shape", SystMap<>::init(1.00));
  src.cp().process_rgx({higgs_rgx, "Ztt"}).bin_id({1, 3})
      .AddSyst(cb, "CMS_scale_e_highpt_$ERA", "shape", SystMap<>::init(1.00));
  ...
  src.cp().era({"7TeV"}).process_rgx({higgs_rgx, "EWK", "ttbar"}).AddSyst(cb,
      "CMS_htt_scale_met_$ERA", "lnN", SystMap<bin_id>::init
      ({0, 1, 2, 3, 4}, 1.05));

  src.cp().era({"8TeV"}).process_rgx({higgs_rgx}).AddSyst(cb,
      "CMS_htt_scale_met_$ERA", "lnN", SystMap<bin_id>::init
      ({0},     0.98)
      ({2},     1.01)
      ({1, 3},  0.99)
      ({4, 5},  0.98));
  src.cp().era({"8TeV"}).process({"EWK", "ttbar"}).AddSyst(cb,
      "CMS_htt_scale_met_$ERA", "lnN", SystMap<bin_id, process>::init
      ({0},   {"ttbar"},            1.01)
      ({1},   {"ttbar"},            0.99)
      ({4},   {"ttbar"},            1.00)
      ({4},   {"EWK"},              0.98)
  ...
  src.cp().process({"WH", "ZH"})
      .AddSyst(cb, "pdf_qqbar", "lnN", SystMap<bin_id>::init
        ({0, 1},            1.01)
        ({2, 3, 4, 5},      1.02));
  ...
  src.cp().process_rgx({"^ggH.*$"})
      .AddSyst(cb, "QCDscale_ggH2in", "lnN", SystMap<era, bin_id>::init
        ({"7TeV"}, {4}, 1.274)
        ({"8TeV"}, {4}, 1.228)
        ({"8TeV"}, {4}, 1.307));
  ...
  src.cp().process({"Ztt"})
      .AddSyst(cb, "CMS_htt_ee_ztt_extrap_$BIN_$ERA", "lnN",
        SystMap<bin_id>::init
        ({1, 2},  1.03)
        ({3},     1.05)
        ({4, 5},  1.10));
  ...
  src.cp().process({"ttbar"})
      .AddSyst(cb, "CMS_htt_ttbarNorm_$BIN_$ERA", "lnN",
  ...
  src.cp().process({"Fakes"})
      .AddSyst(cb, "CMS_htt_fakes_em_$ERA", "lnN", SystMap<>::init(1.30));

  src.cp().process({"Fakes"})
      .AddSyst(cb, "CMS_htt_fakes_$BIN_$ERA", "lnN", SystMap<era, bin_id>::init
        ({"7TeV"},  {1},          1.30)
        ({"7TeV"},  {2},          1.11)
        ({"7TeV"},  {3},          1.05)
        ({"7TeV"},  {4},          1.09)
  ...
  auto em_bins = src.cp().bin_id({0, 1, 2}).bin_set();
  for (auto bin : em_bins) {
    std::string clip = bin;
    boost::replace_all(clip, "emu_", "em_");
    src.cp().process({"Fakes"}).bin({bin}).AddSyst(cb,
      "CMS_htt_FakeShape_" + clip + "_$ERA", "shape", SystMap<>::init(1.00));
  }
}
...
void AddSystematics_et_mt(CombineHarvester & cb) {
  ...
  src.cp().process({"qqH", "WH", "ZH"})
      .AddSyst(cb, "pdf_qqbar", "lnN", SystMap<channel, era, process>::init
  ...
  src.cp().process({"ggH"})
      .AddSyst(cb, "QCDscale_ggH1in", "lnN", SystMap<channel, bin_id>::init
      ({"mt"}, {3}, 1.105)
      ({"mt"}, {4}, 1.095)
      ({"mt"}, {5}, 1.195)
      ({"et"}, {3}, 1.125)
  ...
  src.cp().process({"ggH"})
      .AddSyst(cb, "QCDscale_ggH2in", "lnN", SystMap<channel, era, bin_id>::init
  ...
  src.cp().process({"qqH"})
      .AddSyst(cb, "QCDscale_qqH", "lnN", SystMap<channel, era, bin_id>::init
  ...
  src.cp().process({"WH", "ZH"})
      .AddSyst(cb, "QCDscale_VH", "lnN", SystMap<channel, era, bin_id>::init
  ...
  src.cp().channel({"mt"})
      .process(JoinStr({signal, {"ZTT", "ZL", "ZJ", "TT", "VV"}}))
  ...
  src.cp().process(JoinStr({signal, {"ZTT", "TT", "VV"}})).AddSyst(cb,
    "CMS_eff_t_$CHANNEL_medium_$ERA", "lnN", SystMap<channel, era, bin_id>::init
  ...
  src.cp().process(JoinStr({signal, {"ZTT", "TT", "VV"}}))
      .AddSyst(cb, "CMS_eff_t_$CHANNEL_high_$ERA", "lnN",
  ...
  src.cp().channel({"mt"}).process(JoinStr({signal, {"ZTT"}}))
      .AddSyst(cb, "CMS_scale_t_mutau_$ERA", "shape", SystMap<>::init(1.00));

  src.cp().channel({"et"}).process(JoinStr({signal, {"ZTT"}}))
      .AddSyst(cb, "CMS_scale_t_etau_$ERA", "shape", SystMap<>::init(1.00));
  ...
  src.cp()
      .AddSyst(cb, "CMS_htt_scale_met_$ERA", "lnN",
        SystMap<channel, era, bin_id, process>::init
  ...
  src.cp().process({"ZTT"})
      .AddSyst(cb, "CMS_htt_extrap_ztt_$BIN_$ERA", "lnN",
        SystMap<channel, bin_id>::init
        ({"et", "mt"}, {1, 2, 3, 4, 5},   1.05)
        ({"et", "mt"}, {6},               1.10)
        ({"mt"},       {7},               1.13)
        ({"et"},       {7},               1.16));
  src.cp().channel({"et"}).bin_id({6}).era({"7TeV"}).process({"ZL"})
      .AddSyst(cb, "CMS_htt_extrap_ztt_$BIN_$ERA", "lnN", SystMap<>::init
        (1.10));
  ...
  src.cp().process({"TT"})
      .AddSyst(cb,
      "CMS_htt_ttbarNorm_$BIN_$ERA", "lnN", SystMap<channel, era, bin_id>::init
      ({"mt"},  {"7TeV"}, {6},     1.20)
      ({"mt"},  {"8TeV"}, {6, 7},  1.33)
      ({"et"},  {"7TeV"}, {6},     1.23)
      ({"et"},  {"8TeV"}, {6, 7},  1.33));
  ...
  src.cp().process({"W"})
      .AddSyst(cb, "CMS_htt_WNorm_$BIN_$ERA", "lnN",
        SystMap<channel, era, bin_id>::init
        ({"mt", "et"}, {"7TeV"}, {1, 2},    1.20)
        ({"mt", "et"}, {"7TeV"}, {3, 4},    1.10)
        ({"mt"},       {"7TeV"}, {5},       1.12)
        ({"et"},       {"7TeV"}, {5},       1.10)
  ...
  src.cp().process({"QCD"})
      .AddSyst(cb,
      "CMS_htt_QCDSyst_$BIN_$ERA", "lnN", SystMap<channel, era, bin_id>::init
  ...
  src.cp().channel({"mt"}).process({"QCD"}).bin_id({3})
      .AddSyst(cb, "CMS_htt_QCDShape_mutau_1jet_medium_$ERA", "shape",
        SystMap<>::init(1.00));
  src.cp().channel({"et"}).process({"QCD"}).bin_id({3})
      .AddSyst(cb, "CMS_htt_QCDShape_etau_1jet_medium_$ERA", "shape",
        SystMap<>::init(1.00));

  src.cp().channel({"mt"}).process({"QCD"}).bin_id({4})
      .AddSyst(cb, "CMS_htt_QCDShape_mutau_1jet_high_lowhiggs_$ERA", "shape",
        SystMap<>::init(1.00));

  src.cp().channel({"mt"}).process({"QCD"}).bin_id({6}).era({"7TeV"})
      .AddSyst(cb, "CMS_htt_QCDShape_mutau_vbf_$ERA", "shape",
        SystMap<>::init(1.00));
  src.cp().channel({"et"}).process({"QCD"}).bin_id({6}).era({"7TeV"})
      .AddSyst(cb, "CMS_htt_QCDShape_etau_vbf_$ERA", "shape",
        SystMap<>::init(1.00));

  src.cp().channel({"mt"}).process({"QCD"}).bin_id({6}).era({"8TeV"})
      .AddSyst(cb, "CMS_htt_QCDShape_mutau_vbf_loose_$ERA", "shape",
        SystMap<>::init(1.00));
  src.cp().channel({"et"}).process({"QCD"}).bin_id({6}).era({"8TeV"})
      .AddSyst(cb, "CMS_htt_QCDShape_etau_vbf_loose_$ERA", "shape",
        SystMap<>::init(1.00));
  ...
  src.cp().channel({"mt"}).process({"ZL"})
      .AddSyst(cb, "CMS_htt_ZLeptonFakeTau_$CHANNEL_$ERA", "lnN",
        SystMap<>::init(1.30));


  src.cp().channel({"et"}).process({"ZL"}).bin_id({3, 6, 7})
      .AddSyst(cb, "CMS_htt_ZLeptonFakeTau_$CHANNEL_low_$ERA", "lnN",
        SystMap<bin_id>::init({3}, 1.20)({6, 7}, 1.06));
  src.cp().channel({"et"}).process({"ZL"}).bin_id({3, 5})
      .AddSyst(cb, "CMS_htt_ZLeptonFakeTau_$CHANNEL_met_$ERA", "lnN",
        SystMap<>::init(1.30));
  src.cp().channel({"et"}).process({"ZL"}).bin_id({1, 6, 7})
      .AddSyst(cb, "CMS_htt_ZLeptonFakeTau_$CHANNEL_medium_$ERA", "lnN",
        SystMap<bin_id>::init({1}, 1.20)({6, 7}, 1.08));
  src.cp().channel({"et"}).process({"ZL"}).bin_id({3}).era({"8TeV"})
      .AddSyst(cb, "CMS_htt_ZLeptonFakeTau_$CHANNEL_medium_$ERA", "lnN",
        SystMap<bin_id>::init({3}, 1.20));
  src.cp().channel({"et"}).process({"ZL"}).bin_id({2, 5, 6, 7})
      .AddSyst(cb, "CMS_htt_ZLeptonFakeTau_$CHANNEL_high_$ERA", "lnN",
        SystMap<bin_id>::init({2, 5}, 1.20)({6, 7}, 1.06));
  ...
  src.cp().channel({"et"}).process({"ZL"})
      .AddSyst(cb,
      "CMS_htt_ZLScale_etau_$ERA", "shape", SystMap<>::init(1.00));
}
