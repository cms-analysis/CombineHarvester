...
struct convert_py_root_to_cpp_root {
  ...
  static void construct(PyObject* obj_ptr,
    ...
    // void* storage =
    //     ((py::converter::rvalue_from_python_storage<T>*)data)
    //         ->storage.bytes;
...
struct convert_py_tup_to_cpp_pair {
  convert_py_tup_to_cpp_pair() {
    py::converter::registry::push_back(
        &convertible, &construct, py::type_id<std::pair<T, U> >()
  ...
  static void construct(PyObject* obj_ptr,
    ...
    void* storage =
        ((py::converter::rvalue_from_python_storage<std::pair<T, U> >*)data)
            ->storage.bytes;
    new (storage)
        std::pair<T, U>(py::extract<T>(first), py::extract<U>(second));
    data->convertible = storage;
  }
};
} // anonymous namespace

void FilterAllPy(ch::CombineHarvester & cb, py::object func) {
      auto lambda = [func](ch::Object *obj) -> bool {
      return py::extract<bool>(func(boost::ref(*obj)));
  };
  cb.FilterAll(lambda);
}

void FilterObsPy(ch::CombineHarvester & cb, py::object func) {
      auto lambda = [func](ch::Observation *obs) -> bool {
      return py::extract<bool>(func(boost::ref(*obs)));
  };
  cb.FilterObs(lambda);
}

void FilterProcsPy(ch::CombineHarvester & cb, py::object func) {
      auto lambda = [func](ch::Process *proc) -> bool {
      return py::extract<bool>(func(boost::ref(*proc)));
  };
  cb.FilterProcs(lambda);
}

void FilterSystsPy(ch::CombineHarvester & cb, py::object func) {
      auto lambda = [func](ch::Systematic *sys) -> bool {
      return py::extract<bool>(func(boost::ref(*sys)));
  };
  cb.FilterSysts(lambda);
}

void ForEachObsPy(ch::CombineHarvester & cb, py::object func) {
...
void ForEachProcPy(ch::CombineHarvester & cb, py::object func) {
...
void ForEachSystPy(ch::CombineHarvester & cb, py::object func) {
...
void ForEachObjPy(ch::CombineHarvester & cb, py::object func) {
...
void Overload3_WriteDatacard(ch::CombineHarvester& cb, std::string const& name, py::object& file){
  TFile *file_ = nullptr;
  if (!file.is_none())
    file_ = (TFile*)(CPyCppyy::Instance_AsVoidPtr(file.ptr()));
...
BOOST_PYTHON_MODULE(libCombineHarvesterCombineTools)
{
  ...
  py::class_<CombineHarvester>("CombineHarvester")
      ...
      .def("ExtractData", &CombineHarvester::ExtractData)
