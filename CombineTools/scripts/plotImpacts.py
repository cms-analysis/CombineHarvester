...
parser.add_argument('--auto-page', action='store_true', default=False, help='Automatic number of pages')
...
if args.auto_page:
    ...
    # keydict = dict(zip(all_results['max_impact'], Y))
    neg_sum = 0
    pos_sum = 0
    n_params = len(data['params'])
    for p in xrange(n_params):
        pre = data['params'][p]['prefit']
        fit = data['params'][p]['fit']
        tp = data['params'][p]['type']
        seen_types.add(tp)
        ...
        all_results['pull'].append(pull)
        all_results['pull_lo'].append(pull_lo)
        all_results['pull_hi'].append(pull_hi)

        imp = data['params'][p][POI]
        all_results['g_impacts_lo_err'].append(imp[1] - imp[0])
        all_results['g_impacts_hi_err'].append(imp[2] - imp[1])
        if imp[1] - imp[0] > 0 :
            pos_sum += pow(imp[1] - imp[0], 2)
        else:
            neg_sum += pow(imp[1] - imp[0], 2)
        if imp[2] - imp[1] > 0 :
            pos_sum += pow(imp[2] - imp[1], 2)
        else:
            neg_sum += pow(imp[2] - imp[1], 2)
        all_results['max_impact'].append(max(abs(imp[1] - imp[0]), abs(imp[2] - imp[1])))

    # optimal page-break
    import math
    print 'tot. pos. uncertainty from impacts:', math.sqrt(pos_sum)
    print 'tot. neg. uncertainty from impacts:', math.sqrt(neg_sum)

    max_impact = sorted(all_results['max_impact'], reverse=True)
    last_on_page = []
    nonpage=0
    # n = (len(data['params']) / show) #int(math.log10(x))
    for i in range(0, len(max_impact) - 1):
        if i + 1 == len(max_impact) - 1:
            break
        if nonpage >= show or int(math.log10(max_impact[i + 1])) != int(math.log10(max_impact[i])): #max_impact[i + 1] < max_impact[i]/10:
            if max_impact[i + 1] < max_impact[i]/10:
                print '!break(scale):', max_impact[i + 1] ,"<", max_impact[i]/10, '(', int(math.log10(max_impact[i + 1])), int(math.log10(max_impact[i])), ')'
            else:
                print '!break(nmax):', nonpage ,">=", show
            last_on_page.append(i)
            nonpage = 0
            continue
        else:
            print 'nobreak:', max_impact[i + 1] ,">=", max_impact[i]/10
        nonpage += 1
    last_on_page.append(len(max_impact))
    n = len(last_on_page)
    # update margin when 10^x appear
    updatemargin = False
    if int(math.log10(min(all_results['max_impact'])))<=2 :
        print "changing margin"
        updatemargin = True
    # print max_impact
    # print 'nparam:', len(data['params'])
    # print last_on_page ; exit(1)
    ...
    for page in xrange(n):
        ...
        if page == 0:
            start = 0
            if last_on_page[0] == 0:
                n_params = len(data['params'][0:1])
                pdata = data['params'][0:1]
                last_on_page[0] = 1
            else:
                n_params = len(data['params'][0:last_on_page[page]])
                pdata = data['params'][0:last_on_page[page]]
        else:
            start = last_on_page[page - 1]
            n_params = len(data['params'][last_on_page[page - 1]:last_on_page[page]])
            pdata = data['params'][last_on_page[page - 1]:last_on_page[page]]
        # show = len(n_params)

        # n_params = len(data['params'][show * page:show * (page + 1)])
        # pdata = data['params'][show * page:show * (page + 1)]
        print '>> Doing page %i, have %i parameters, indexed [%i, %i)' % (page, n_params, start, last_on_page[page])
        ...
        for i in xrange(n_params):
            ...
            if i % 2 == 0:
                box.SetFillColor(18)
            box.AddText('%i' % (n_params - i + start))
            box.Draw()
            boxes.append(box)
        ...
        pads[1].SetTickx(1)
        if updatemargin:
            pads[1].SetRightMargin(pads[1].GetRightMargin() + 0.025)
...
for page in xrange(n):
    ...
    for p in xrange(n_params):
        ...
        max_impact = max(
            max_impact, abs(imp[1] - imp[0]), abs(imp[2] - imp[1]))
        min_impact = min(
            min_impact, abs(imp[1] - imp[0]), abs(imp[2] - imp[1]))
        col = colors.get(tp, 2)
        if args.color_groups is not None and len(pdata[p]['groups']) >= 1:
            for p_grp in pdata[p]['groups']:
                if p_grp in color_groups:
    ...
    if int(math.log10(min_impact)) <= 2:
        pads[1].SetRightMargin(pads[1].GetRightMargin() + 0.025)
