...
void ReverseBins(TH2F & h) {
  std::vector<float> contents(h.GetNbinsX()*h.GetNbinsY());
  std::vector<float> errors(h.GetNbinsX()*h.GetNbinsY());
  for (int j = 0; j < h.GetNbinsY(); ++j) {
    for (int i = 0; i < h.GetNbinsX(); ++i) {
      contents[j*h.GetNbinsX()+i] = h.GetBinContent(i + 1, j + 1);
      errors[j*h.GetNbinsX()+i] = h.GetBinError(i + 1, j + 1);
    }
  }
  for (int j = 0; j < h.GetNbinsY(); ++j) {
    for (int i = 0; i < h.GetNbinsX(); ++i) {
      h.SetBinContent(h.GetNbinsX()*h.GetNbinsY() - j*h.GetNbinsX()+i, contents[j*h.GetNbinsX()+i]);
      h.SetBinError(h.GetNbinsX()*h.GetNbinsY() - j*h.GetNbinsX()+i, errors[j*h.GetNbinsX()+i]);
    }
  }
  // return h;
}
...
int main(int argc, char* argv[]) {
  ...
  if (!skip_prefit) {
    ...
    for (auto bin : bins) {
      ...
      pre_shapes[bin]["data_obs"] = cmb_bin.GetObserved2DShape();
      for (auto proc : cmb_bin.process_set()) {
        std::cout << ">> Doing prefit: " << bin << "," << proc << std::endl;
        if (skip_proc_errs) {
          pre_shapes[bin][proc] =
              cmb_bin.cp().process({proc}).Get2DShape();
        } else {
          pre_shapes[bin][proc] =
              cmb_bin.cp().process({proc}).Get2DShapeWithUncertainty();
        }
      }
      ...
      pre_shapes[bin]["TotalBkg"] =
          cmb_bin.cp().backgrounds().Get2DShapeWithUncertainty();
      std::cout << ">> Doing prefit: " << bin << "," << "TotalSig" << std::endl;
      pre_shapes[bin]["TotalSig"] =
          cmb_bin.cp().signals().Get2DShapeWithUncertainty();
  ...
  if (postfit) {
    ...
    for (auto bin : bins) {
      ch::CombineHarvester cmb_bin = cmb.cp().bin({bin});
      post_shapes[bin]["data_obs"] = cmb_bin.GetObserved2DShape();
      for (auto proc : cmb_bin.process_set()) {
