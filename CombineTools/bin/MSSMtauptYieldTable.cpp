...
// Struct holds info on each signal row in the table
struct SigInfo {
  string label;          // Latex title, e.g. "$\\PW$+jets"
  vector<string> procs;  // Datacard processes to combine for this entry
  SigInfo(string const& l, vector<string> const& p) : label(l), procs(p) {}
};
...
int main(int argc, char* argv[]) {
  ...
  vector<SigInfo> sigs;
  sigs = {
      SigInfo("gg(h+A+H)",       {"ggH"}),
      SigInfo("bb(h+A+H)",       {"bbH"}),
    };
  ...
  if (channel == "et" || channel == "mt" || channel == "tt") {
    bkgs = {
      ...
      BkgInfo("SM Higgs (125 GeV)",                   {"ggH_SM125", "qqH_SM125", "VH_SM125"})
    };
    total_bkg = {"ZTT", "QCD", "W", "ZL", "ZJ", "TT", "VV", "ggH_SM125", "qqH_SM125", "VH_SM125"};
  }
  if (channel == "em") {
    bkgs = {
      BkgInfo("$\\cPZ\\rightarrow \\Pgt\\Pgt$",    {"Ztt"}),
    ...
    total_bkg = {"Ztt", "Fakes", "ttbar", "EWK", "ggH_SM125", "qqH_SM125", "VH_SM125"};
  }
  if (channel == "mm") {
    bkgs = {
      BkgInfo("$\\cPZ\\rightarrow \\Pgt\\Pgt$",    {"ZTT"}),
    ...
    total_bkg = {"ZTT", "ZMM", "QCD", "TTJ", "WJets", "Dibosons", "ggH_SM125", "qqH_SM125", "VH_SM125"};
  }
  ...
  double sig_yields[n_cols][n_sig];
  double sig_errors[n_cols][n_sig];
  ...
  for (unsigned i = 0; i < n_cols; ++i) {
    ...
    for (unsigned j = 0; j < n_sig; ++j) {
      sig_yields[i][j] = tmp_cmb.cp().process(sigs[j].procs).GetRate();
      sig_errors[i][j] = postfit ?
        tmp_cmb.cp().process(sigs[j].procs).GetUncertainty(fitresult, samples) :
        tmp_cmb.cp().process(sigs[j].procs).GetUncertainty();
    }
  ...
  cout << boost::format("%-38s") % "gg(A+H+h)\\,$\\rightarrow\\Pgt\\Pgt$";
  for (unsigned i = 0; i < n_cols; ++i)
    cout << boost::format(sig_errors[i][0] < 1. ? fmts : fmt) % sig_yields[i][0] %
                sig_errors[i][0];

  cout << "\\\\\n";

  cout << boost::format("%-38s") % "bb(A+H+h)\\,$\\rightarrow\\Pgt\\Pgt$";
  for (unsigned i = 0; i < n_cols; ++i)
    cout << boost::format(sig_errors[i][1] < 1. ? fmts : fmt) % sig_yields[i][1] %
                sig_errors[i][1];
  ...
  cout << boost::format(
              ...
              (n_cols*2 + 1);
  cout << "\\hline\n";
  cout << boost::format("%-38s") % (boost::format("gluon-fusion Higgs (%s~\\GeV)") % signal_mass);
  for (unsigned i = 0; i < n_cols; ++i) {
  ...
  cout << boost::format("%-38s") % (boost::format("b-associated Higgs (%s~\\GeV)") % signal_mass);
  for (unsigned i = 0; i < n_cols; ++i) {
